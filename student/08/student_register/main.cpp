...
void save_data(const map<string, Student*>& data, const string& file_name)
{
    ofstream data_file(file_name);
    for (auto x : data)
    {
        data_file << x.second->student_number << ";" << x.second->user_id << ";"
                  << x.second->name << ";" << x.second->phone_number << ";"
                  << x.second->email << ";" <<  x.second->skype << "\n";
    }
    data_file.close();
}

void change_phone_number(map<string, Student*>& numbers_map, map<string, Student*>& id_map, const string& student_number, const string& file_name)
{
    if (numbers_map.find(student_number) == numbers_map.end())
    {
    ...
    string user_id = numbers_map.at(student_number)->user_id;

    numbers_map.at(student_number)->phone_number = phone_num;
    id_map.at(user_id)->phone_number = phone_num;

    save_data(id_map, file_name);
}
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            change_phone_number(student_numbers, user_ids, parts[1], file_name);
